...
// 工作目录设置
const WORK_DIR = process.env.WORK_DIR || "C:\\Users\\hrp\\Downloads\\ai-pro\\data\\arxiv";

// LLM API 配置 (使用自定义 API)
const LLM_API_ENDPOINT = process.env.LLM_API_ENDPOINT || "https://alex-ewoeo-bigeowifo.hf.space/api/coding/paas/v4";
const LLM_API_KEY = process.env.LLM_API_KEY || "A6uYVA1EosYnHrl0lT1r2u2IuEh1AHzAzn-knWY-Dws";
const LLM_MODEL = process.env.LLM_MODEL || "glm-4.7";
const LLM_MAX_TOKENS = parseInt(process.env.LLM_MAX_TOKENS || "4096");
...
// 工具函数：调用自定义 LLM API
async function callLLMAPI(prompt: string, systemPrompt?: string): Promise<string> {
  try {
    ...
    // 构建请求 URL (endpoint + /chat/completions)
    const apiUrl = LLM_API_ENDPOINT.endsWith('/')
    ...
    // 兼容不同 API 响应格式
    return response.data.choices?.[0]?.message?.content || response.data.output?.choices?.[0]?.message?.content || "No response";
  } catch (error) {
    console.error("调用 LLM API 时出错:", error);
...
- 总结性的结尾`;
    ...
    const wechatContent = await callLLMAPI(prompt, systemPrompt);
...
async function parsePdfToMarkdown(pdfPath: string, arxivId: string, paperInfo?: any): Promise<string> {
  try {
    ...
    const markdown = await callLLMAPI(prompt, systemPrompt);
...
console.log(`✅ LLM API: ${LLM_API_ENDPOINT}`);
